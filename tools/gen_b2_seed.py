...
# -*- coding: utf-8 -*-
"""
Sinh migration Flyway V37__seed_b2_flashcards.sql từ vocabulary_oxford5000.json.

- Lọc cefr == "B2" (1295 từ, bỏ Grammar 104 -> 1191 nội dung).
- Bỏ "Grammar & Function Words".
- Gom topic thành 14 BỘ B2 hệ thống (owner_id = NULL), cap 50 từ/bộ
  => tự nhiên cắt còn ~700 từ (chọn từ đủ thông tin nhất qua completeness_score).
- UUID cố định (uuid5) => idempotent.

KHÁC V33 (A1): không đụng index (V33 đã thay uq_desk_global_cefr ->
uq_desk_global_cefr_title). V37 chỉ INSERT — cùng kiểu V35/V36.

Chạy 1 lần:
    python tools/gen_b2_seed.py
Output: src/main/resources/db/migration/V37__seed_b2_flashcards.sql
"""
...
OUT = os.path.join(ROOT, "src", "main", "resources", "db", "migration",
                   "V37__seed_b2_flashcards.sql")

# Namespace riêng cho B2 (KHÁC A1/A2/B1).
NS = uuid.UUID("b2b2b2b2-0000-4000-8000-000000000004")

CEFR = "B2"
MAX_PER_DECK = 50
EXCLUDE_TOPICS = {"Grammar & Function Words"}

# sort_order tiếp nối: A1 1..14, A2 15..28, B1 29..42 -> B2 43..56.
# Phân bố B2 (bỏ Grammar): Emotions 138, Society 122, Business 89, Appearance 87,
#   Education 80, Health 75, Law 72, Arts 67, Work 67, Science 67, Home 51,
#   Nature 47, Tech 44, Time 36, Sports 31, Numbers 31, Travel 26, Shopping 18,
#   Food 14, Family 9, Greetings 9, General Action 5, Weather 3, General 2, Communication 1.
# Topic lớn (>50) tự bị cap 50. Gộp topic nhỏ + nhãn rác (General*) cho đủ bộ.
DECKS = [
    ("B2 · Cảm xúc & Tính cách",        ["Emotions & Feelings"],                             43),
    ("B2 · Xã hội & Văn hóa",           ["Society & Culture"],                               44),
    ("B2 · Kinh doanh & Kinh tế",       ["Business & Economy"],                              45),
    ("B2 · Ngoại hình & Mô tả",         ["Appearance & Description"],                         46),
    ("B2 · Học tập & Trường lớp",       ["Education & Learning"],                            47),
    ("B2 · Cơ thể & Sức khỏe",          ["Health & Body"],                                   48),
    ("B2 · Luật pháp & Chính trị",      ["Law & Politics"],                                  49),
    ("B2 · Giải trí & Nghệ thuật",      ["Arts & Entertainment"],                            50),
    ("B2 · Công việc & Sự nghiệp",      ["Work & Career"],                                   51),
    ("B2 · Khoa học & Nghiên cứu",      ["Science & Research"],                              52),
    ("B2 · Nhà cửa & Đời sống",         ["Home & Living"],                                   53),
    ("B2 · Thiên nhiên & Thời tiết",    ["Nature & Environment", "Weather & Climate"],       54),
    ("B2 · Công nghệ & Di chuyển",      ["Technology & Media", "Travel & Transport",
                                         "Shopping & Money"],                                55),
    ("B2 · Đời sống thường ngày",       ["Time & Daily Routine", "Sports & Hobbies",
                                         "Numbers & Measurement", "Food & Drink",
                                         "Family & Relationships", "Greetings & Communication",
                                         "Communication", "General Action", "General"],      56),
]
...
def main():
    ...
    lines = []
    lines.append("-- ============================================================")
    lines.append("-- V37: Seed %d bộ thẻ B2 (owner_id NULL = desk hệ thống dùng chung)." % len(deck_rows))
    lines.append("-- Sinh tự động từ vocabulary_oxford5000.json (cefr=B2) bằng tools/gen_b2_seed.py.")
    lines.append("-- Bỏ topic 'Grammar & Function Words'. Mỗi bộ <= %d từ. Tổng: %d từ." % (MAX_PER_DECK, total))
    lines.append("-- (Nguồn B2 ~1191 từ nội dung -> cap 50/bộ để giữ ~14 bộ như A1-B1.)")
    lines.append("-- UUID cố định (uuid5) => chạy lại an toàn (ON CONFLICT DO NOTHING).")
    lines.append("-- Index uq_desk_global_cefr_title đã tạo ở V33 (A1) => V37 chỉ INSERT.")
    lines.append("-- ============================================================")
    lines.append("")
