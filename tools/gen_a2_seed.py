...
# -*- coding: utf-8 -*-
"""
Sinh migration Flyway V35__seed_a2_flashcards.sql từ vocabulary_oxford5000.json.

- Lọc cefr == "A2".
- Bỏ topic "Grammar & Function Words" (mạo từ / giới từ / đại từ — không hợp flashcard từ vựng).
- Gom các topic nguồn thành các BỘ (mỗi bộ = 1 Desk A2 hệ thống, owner_id = NULL).
...
uq_desk_global_cefr_title rồi). V35 chỉ INSERT.

Chạy 1 lần (không thuộc runtime app):
    python tools/gen_a2_seed.py
Output: src/main/resources/db/migration/V35__seed_a2_flashcards.sql
"""
...
OUT = os.path.join(ROOT, "src", "main", "resources", "db", "migration",
                   "V35__seed_a2_flashcards.sql")

# Namespace cố định riêng cho A2 -> uuid5 deterministic (KHÁC namespace A1).
NS = uuid.UUID("a2a2a2a2-0000-4000-8000-000000000002")
...
# Định nghĩa các bộ A2: (title hiển thị, [danh sách topic nguồn gộp vào], sort_order).
# Sort_order tiếp nối A1 (A1 dùng 1..14) -> A2 dùng 15.. để xếp sau A1 khi sort chung.
# Phân bố A2 (đã đếm): Appearance 62, Emotions 53, Health 53, Home 50, Society 48,
#   Arts 42, Sports 42, Tech 41, Education 38, Work 35, Nature 31, Travel 30,
#   Science 29, Business 22, Greetings 22, Numbers 21, Law 20, Food 18, Time 16,
#   Shopping 13, Family 10, Weather 9. Gộp topic nhỏ cho đủ ~50/bộ.
DECKS = [
    ("A2 · Ngoại hình & Mô tả",        ["Appearance & Description"],                         15),
    ("A2 · Cảm xúc & Tính cách",       ["Emotions & Feelings"],                              16),
    ("A2 · Cơ thể & Sức khỏe",         ["Health & Body"],                                    17),
    ("A2 · Nhà cửa & Đồ đạc",          ["Home & Living"],                                    18),
    ("A2 · Xã hội & Văn hóa",          ["Society & Culture"],                                19),
    ("A2 · Giải trí & Nghệ thuật",     ["Arts & Entertainment"],                             20),
    ("A2 · Thể thao & Sở thích",       ["Sports & Hobbies"],                                 21),
    ("A2 · Công nghệ & Truyền thông",  ["Technology & Media"],                               22),
    ("A2 · Học tập & Trường lớp",      ["Education & Learning"],                             23),
    ("A2 · Công việc & Sự nghiệp",     ["Work & Career", "Business & Economy"],              24),
    ("A2 · Thiên nhiên & Thời tiết",   ["Nature & Environment", "Weather & Climate"],        25),
    ("A2 · Di chuyển & Mua sắm",       ["Travel & Transport", "Shopping & Money"],           26),
    ("A2 · Khoa học & Luật pháp",      ["Science & Research", "Law & Politics"],             27),
    ("A2 · Chào hỏi, Gia đình & Đời sống",
                                       ["Greetings & Communication", "Family & Relationships",
                                        "Numbers & Measurement", "Food & Drink",
                                        "Time & Daily Routine"],                             28),
]
...
def main():
    ...
    a2 = [w for w in data if str(w.get("cefr", "")).upper() == "A2"]
    ...
    # Cảnh báo topic A2 chưa được gán vào bộ nào (trừ loại trừ).
    ...
    lines = []
    lines.append("-- ============================================================")
    lines.append("-- V35: Seed %d bộ thẻ A2 (owner_id NULL = desk hệ thống dùng chung)." % len(deck_rows))
    lines.append("-- Sinh tự động từ vocabulary_oxford5000.json (cefr=A2) bằng tools/gen_a2_seed.py.")
    ...
    lines.append("-- Index uq_desk_global_cefr_title đã tạo ở V33 (A1) => V35 chỉ INSERT.")
    lines.append("-- ============================================================")
    lines.append("")
    ...
    for title, sort, picked in deck_rows:
        ...
        lines.append("VALUES ('%s', NULL, 'A2', %s, %d)" % (did, sql_str(title), sort))
        ...
        for w in picked:
            ...
            row = "    ('%s', '%s', %s, 'A2', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)" % (
                cid, did,
                sql_str(word),
                sql_json(w.get("pos")),
    ...
    for title, sort, picked in deck_rows:
        print("  [%2d] %-38s %3d tu" % (sort, title.encode("ascii", "replace").decode(), len(picked)))
    if leftover:
        print("Topic A2 CHUA gan vao bo nao (con du):")
